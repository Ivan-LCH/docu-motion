...
function SlideCard({ slide, index, total, projectId, onUpdate, onDelete, onMoveUp, onMoveDown, onPreviewImage, isSelected, onToggleSelect }: {
  ...
  const [trimStart, setTrimStart] = useState<number | ''>(slide.trim_start || '')
  const [trimEnd, setTrimEnd] = useState<number | ''>(slide.trim_end || '')
  ...
  const handleTrimStartBlur = () => {
    const val = typeof trimStart === 'number' ? trimStart : 0.0
    if (val !== slide.trim_start) onUpdate({ ...slide, trim_start: val })
  }
  ...
  const handleTrimEndBlur = () => {
    const val = typeof trimEnd === 'number' ? trimEnd : 0.0
    if (val !== slide.trim_end) onUpdate({ ...slide, trim_end: val })
  }
        ...
        <div style={{ borderRadius: 'var(--radius-sm)', overflow: 'hidden', background: 'var(--bg-secondary)', aspectRatio: '16/9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {slide.image_filename ? (
            <img
              ...
              alt={`Slide ${index + 1}`}
              style={{ width: '100%', height: '100%', objectFit: 'contain', cursor: onPreviewImage ? 'pointer' : 'default' }}
        ...
        {isVideo ? (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>✂️ 자르기</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>시작:</span>
                <input
                  type="number" className="input" style={{ width: 70, padding: '2px 4px', fontSize: '0.78rem' }}
                  step="0.01" min="0" placeholder="0.00"
                  value={trimStart}
                  onChange={handleTrimStartChange}
                  onBlur={handleTrimStartBlur}
                />
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>초</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>종료:</span>
                <input
                  type="number" className="input" style={{ width: 70, padding: '2px 4px', fontSize: '0.78rem' }}
                  step="0.01" min="0" placeholder="0.00"
                  value={trimEnd}
                ...
                />
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>초 (0은 끝까지)</span>
              </div>
            </div>
            {/* Subtitle Row Editor */}
            <div>
...
export default function Editor() {
  ...
  const handleCreateCollage = async () => {
    ...
    try {
      ...
      const newSlide = await api.createCollage(projectId, slideIds)
        ...
        {slides.length === 0 ? (
          ...
          <>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', alignItems: 'center' }}>
              ...
              {selectedSlideIds.size >= 2 && selectedSlideIds.size <= 3 && (
                <button
