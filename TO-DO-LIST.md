...
──────────────────────────────────────────────────────────────────────────────
  ...
  - [O] 6-10. [Editor.tsx] 전역 설정(Master) 전용 관리 메뉴 구축
  ...
  - [O] 6-11. [Editor.tsx] 미디어 업로드 및 버튼 UI 현대화 (디자인 개편)
        . 텍스트 위주 버튼 -> 아이콘+카드 형태의 세련된 디자인으로 교체
        . 전체적인 UI/UX 가시성 및 심미성 강화 (모던 앱 스타일)
        . 완료: action-card, upload-zone, btn-action-primary, slide-ctrl-btn 등 CSS 클래스 추가, 사이드바 전면 리디자인
  - [O] 6-12. [Editor.tsx] 자막 편집 UX 개선 (타임라인 싱크 및 직관적 입력)
        . 현재 시작/종료 시간 수동 입력 방식의 불편함 개선
        . 영상 싱크에 맞게 직관적으로 조절하거나 타임라인에서 제어하는 방식 검토
        . 완료: SubtitleTimeline 드래그 컴포넌트 구현(이동/리사이즈), 초 단위 숫자+range slider 병행 입력, 타임라인 클릭 시 비디오 시크 연동
